...
async function getToken() {
  ...
  const server = await axios.get("https://open.spotify.com/api/server-time", { headers: { Cookie:`sp_dc=${SP_DC}` } })
  ...
  try {
    ...
    try {
      const response = await axios.post(
        ...
        body,
        { headers: { Authorization:`Bearer ${token}`, "Content-Type":"application/x-protobuf" }, responseType:"arraybuffer" }
      );
    ...
    // fallback album art
    const meta = await axios.get(`https://api.spotify.com/v1/tracks/${trackId}`, { headers:{Authorization:`Bearer ${token}`} });
...
app.get("/lyric", async (req,res)=>{
  ...
  try {
    const token = await getToken();
    const response = await axios.get(`https://spclient.wg.spotify.com/color-lyrics/v2/track/${trackId}`, {
    ...
    const lyrics = lines.map(line=>({ startTimeMs:line.startTimeMs, words:line.words }));
